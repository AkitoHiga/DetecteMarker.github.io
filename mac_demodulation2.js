...
function successCallback(stream) {
  ...
  */
  /*    04/13.406テスト環境
  const pilot_S = [
    [[63, 157, 15], [164, 247, 122], [57, 148, 155]], 
    [[90, 253, 122], [0, 0, 0], [161, 253, 4]], 
  ...
  */
  //  04/24.ニューラルテスト環境
  const pilot_S = [
    [[142, 167, 179], [214, 246, 252], [157, 197, 238]], 
    [[186, 241, 252], [170, 172, 186], [171, 199, 171]], 
    [[146, 135, 144], [120, 177, 147], [181, 189, 217]],
  ];
          ...
          for(let px=0; px<ave_position[0].length; px++){
            for(let y=0; y<ave_area; y++){
              for(let x=0; x<ave_area; x++){
                ave_temp = add(ave_temp,videoMatNow.ucharPtr(anky+parseInt(size_y/6)*(py*2+1)-parseInt(ave_area/2)+x, ankx+parseInt(size_x/6)*(px*2+1)-parseInt(ave_area/2)+y));
              }
            }
          ...
          //for(let px=0; x<ave_position[0].length; x++){
            for(let y=0; y<ave_area; y++){
              for(let x=0; x<ave_area; x++){
                ...
                ave_temp = add(ave_temp,videoMatNow.ucharPtr(anky+parseInt(size_y/3)*(py)-parseInt(ave_area/2)+y, ankx+parseInt(size_x/3)*(px)-parseInt(ave_area/2)+x));
              }
            }
            ave_position[py][px] = div(ave_temp,ave_area*ave_area);
            //ave_temp = [0,0,0];
